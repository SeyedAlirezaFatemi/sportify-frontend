...
class HomePage extends React.Component<any, any> {
      ...
      <Template>
        <Tabs defaultActiveKey="1" tabBarStyle={{ textAlign: 'center' }}>
          <TabPane tab="Soccer" key="1">
            {this.renderContent(API.LATEST_NEWS_SOCCER, Sports.SOCCER)}
          </TabPane>
          <TabPane tab="Basketball" key="2">
            {this.renderContent(API.LATEST_NEWS_BASKETBALL, Sports.BASKETBALL)}
          </TabPane>
        </Tabs>
      </Template>
    )
  ...
  private renderContent(url: string, sport: Sports) {
    const { classes } = this.props;
    return (
